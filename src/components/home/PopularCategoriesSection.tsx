"use client";

import Link from "next/link";
import { Swiper, SwiperSlide } from "swiper/react";
import { FreeMode } from "swiper/modules";
import { proxyImageUrl } from "@/lib/utils/image";
import "swiper/css";
import "swiper/css/free-mode";
import type { Category } from "@/lib/api/categories";

interface Props {
  categories: Category[];
}

const PopularCategoriesSection = ({ categories }: Props) => {
  if (categories.length === 0) return null;

  return (
    <section className="mb-6 md:mb-10 overflow-x-hidden overflow-y-hidden">
      <h2 className="text-base md:text-xl font-bold text-[#1D1D1D] mb-3 md:mb-5 px-1">
        Most Popular Category
      </h2>
      <Swiper
        modules={[FreeMode]}
        freeMode={true}
        spaceBetween={12}
        slidesPerView="auto"
        breakpoints={{
          640:  { spaceBetween: 16 },
          768:  { spaceBetween: 20 },
          1024: { spaceBetween: 24 },
          1280: { spaceBetween: 24 },
        }}
        className="!overflow-visible"
        preventClicks={false}
        preventClicksPropagation={false}
      >
        {categories.map((category) => (
          <SwiperSlide key={category.id} className="!w-auto">
            <Link
              href={`/products?categories=${category.id}`}
              className="flex flex-col items-center gap-1.5 group active:scale-95 md:hover:scale-105 transition-transform duration-300"
            >
              <div className="rounded-xl w-[90px] h-[90px] sm:w-[110px] sm:h-[110px] md:w-[130px] md:h-[130px] bg-[#EAFCFF] flex items-center justify-center group-active:bg-[#DDF8FD] md:group-hover:bg-[#DDF8FD] transition-colors duration-300 shadow-md overflow-hidden">
                <img
                  src={proxyImageUrl(category.image_full_path)}
                  alt={category.name}
                  className="w-full h-full object-contain"
                  loading="lazy"
                />
              </div>
              <span className="text-[11px] sm:text-xs md:text-sm font-medium text-[#1D1D1D] text-center whitespace-nowrap">
                {category.name}
              </span>
            </Link>
          </SwiperSlide>
        ))}
      </Swiper>
    </section>
  );
};

export default PopularCategoriesSection;
